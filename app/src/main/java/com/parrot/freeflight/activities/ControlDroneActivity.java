        ...
        DroneBatteryChangedReceiverDelegate, DroneFlyingStateReceiverDelegate, DroneCameraReadyActionReceiverDelegate, DroneRecordReadyActionReceiverDelegate, SettingsDialogDelegate
{
    ...
    private final int OLDRANGE = 180;
    private final int NEWRANGE = 2;
   ...
   // private boolean forceCombinedControlMode;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        if (!hub.init(this, getPackageName())) {
            ...
            Log.e(TAG, "Couldn't initialize Hub");
            finish();
            return;
        }
        Log.w(TAG,"Hub");
...
//            forceCombinedControlMode = false;
        ...
        if (!hub.init(this, getPackageName())) {
            // We can't do anything with the Myo device if the Hub can't be initialized, so exit.
            Log.w(TAG, "Couldn't initialize Hub");
            finish();
            return;
        }
        Log.w(TAG,"Hub");
        ...
        mListener = new AbstractDeviceListener(){
            ...
            public void onOrientationData(Myo myo, long timestamp, Quaternion rotation) {
                if(myo.isUnlocked())
                {
                    ...
                    //convert oreintation values to range -1 to 1
                    roll = (((roll - 0) * NEWRANGE) / OLDRANGE) - 1;
                    pitch = (((pitch - 0) * NEWRANGE) / OLDRANGE) - 1;
                    yaw = (((yaw - 0) * NEWRANGE) / OLDRANGE) - 1;


                    if (droneControlService != null)
                    {
                        droneControlService.setGaz(pitch);
                        droneControlService.setRoll(yaw);
                        droneControlService.setPitch(roll);
                    }
                }
            ...
            public void onPose(Myo myo, long timestamp, Pose pose) {
                switch (pose) {
                    ...
                    case DOUBLE_TAP:
                        if (myo.isUnlocked()) {
                            if (droneControlService != null){
                                droneControlService.setControls(0, 0, 0, 0, 0, 0);
                            }
                    ...
                    case WAVE_IN:
                        if (droneControlService != null){
                            droneControlService.turnLeft(1 / 2);
                        }

                        break;
                    case WAVE_OUT:
                        if (droneControlService != null){
                            droneControlService.turnRight(1 / 2);
                        }
